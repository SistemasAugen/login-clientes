			...
			<div class="panel panel-primary" data-collapsed="0">
				...
				<div class="panel-heading">
					...
					<div class="panel-options">
						<a @click="$router.push('/brands/')"><i class="fas fa-times"></i></a>
					</div>
				</div>
					...
					<form role="form" class="form-horizontal" @submit.prevent="newRow($event.target)">
						...
						<div class="form-group">
							<div class="col-sm-12">
								...
								<button type="button" class="btn btn-default pull-right" @click="$router.push('/brands/')">Cancelar</button>
							</div>
						</div>
	...
	export default {
		...
		methods:{
			...
			getRow(){
				...
				axios.get(tools.url("/api/admin/brands/"+this.id)).then((response)=>{
			...
			newRow(form){
				...
				this.$parent.validateAll(()=>{
					...
					if(this.$route.params.id){
						axios.post(tools.url("/api/admin/brands/"+this.id),data)
						.then((response)=>{
					    	this.getRow();
						...
						axios.post(tools.url("/api/admin/brands"),data).then((response)=>{
							var temp = response.data;
					    	this.$parent.showMessage("Registro agregado correctamente!","success");
					    	this.$router.push('/brands/edit/'+temp.id);
					    	this.$parent.inPetition=false;
					    }).catch((error)=>{
					    	this.$parent.handleErrors(error);
					        this.$parent.inPetition=false;
			...
			deleteRow:function(){
				alertify.confirm("Alerta!","¿Seguro que deseas borrar?",()=>{
					this.$parent.inPetition=true;
					axios.delete(tools.url("/api/admin/brands/"+this.id))
					.then((response)=>{
